...
serve(async (req) => {
  ...
  try {
    const { merchantId, originalAmount, grabId, dealId, anonymousUserId, isDemoMode, localCreditsUsed = 0, networkCreditsUsed = 0, amountEntryMode = 'customer' } = await req.json();
    ...
    // Validate required fields - originalAmount is optional for merchant-keyed flow
    if (!merchantId) {
      return new Response(
