...
-- Update existing pending_transactions status values to be compatible
UPDATE pending_transactions SET status = 'awaiting_customer' WHERE status = 'pending';

-- Add status constraint after updating data
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'valid_status' AND table_name = 'pending_transactions'
  ) THEN
